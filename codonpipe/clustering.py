...
def _compute_modification_usage_z(count_df: pd.DataFrame, trna_rules: dict, feature_mode: str = 'modifications'):
    """Return per-gene z-scored usage of codons annotated with each modification or tRME.

    feature_mode='modifications' uses column F of the unified decoding table.
    feature_mode='enzymes' uses column G. Each comma/semicolon-separated item is
    treated as a separate feature, so labels are no longer merged as
    "modification (enzyme1, enzyme2, ...)".
    """
    ...
    if feature_mode in {'enzyme', 'enzymes', 'trme', 'trmes', 'trme enzymes'}:
        by_codon = trna_rules.get('trmes_by_codon') or {}
        ylabel = 'tRNA modification enzymes'
    else:
        by_codon = trna_rules.get('modifications_by_codon') or {}
    ...
    for codon, items in by_codon.items():
        ...
        for item in list(items or []):
            ...
            if not feat:
                continue
            feature_to_codons.setdefault(feat, [])
            if codon not in feature_to_codons[feat]:
                feature_to_codons[feat].append(codon)
    if not feature_to_codons:
        return pd.DataFrame(), {}

    total = _safe_total_sense_counts(count_df)
    raw = pd.DataFrame(index=count_df.index)
    for feat, codons in feature_to_codons.items():
        codons = [c for c in dict.fromkeys(codons) if c in count_df.columns]
        if not codons:
            continue
        raw[feat] = count_df.loc[:, codons].apply(pd.to_numeric, errors='coerce').sum(axis=1).div(total) * 100.0
    if raw.empty:
        return raw, {}
...
def _plot_cluster_feature_boxplots(
    ...
    caption_size = int(SET.get(f'{key_prefix}_boxplot_caption_size', 13) or 13)
    ...
    for i in range(n_features):
        if i == 0:
            ...
            pos += 1.0 + (1.05 if has_groups and cur and prev and cur != prev else (0.38 if plot_key in {'modifications', 'modification'} else 0.0))
    ...
    fig_w = float(fig_w if fig_w is not None else max(8.2, 0.78 * (xbase[-1] + 1 if len(xbase) else n_features) + 3.8))
    fig_h = float(fig_h if fig_h is not None else max(5.3, 4.1 + 0.08 * n_clusters))
    # A single per-plot caption size controls title, labels, ticks, and legend.
    ...
    if feature_metadata:
        ...
        for i, f in enumerate(features):
            ...
            txt = ax.text(xbase[i], float(top_y), top, ha='center', va='top', transform=trans,
                          fontsize=caption_size, color=top_color, clip_on=False)
                ...
                if i == n_features or group_values[i] != group_values[start]:
                    ...
                    if grp:
                        ...
                        ax.text(0.5 * (x0 + x1), group_label_y, group_label, ha='center', va='top', transform=trans,
                                fontsize=caption_size, fontweight='bold', clip_on=False)
                    start = i
    ...
    # Titles are intentionally suppressed for Decoding strategies figures,
    # especially when a legend is placed above the graph.
    ax.set_title('')
    ...
    if ref_stats_enabled and reference_cluster in clusters:
        ...
        for i_f, f in enumerate(features):
            ...
            for j, c in enumerate(clusters):
                ...
                ax.text(xpos, ypos, stars, ha='center', va='bottom', fontsize=caption_size, fontweight='bold', clip_on=False, zorder=8)
    ...
    # row, so cluster legends wrap cleanly when many clusters are displayed.
    legend_ncol = max(1, min(4, len(handles)))
    legend_rows = int(math.ceil(len(handles) / float(legend_ncol))) if handles else 1
    # Titles are suppressed, so place the legend closer to the axes and use
    # the space that was previously occupied by the title.
    ...
    try:
        ...
        bottom_margin = 0.32 if feature_metadata else 0.20
        top_margin = max(0.66, 0.89 - 0.050 * max(0, legend_rows - 1))
        fig.subplots_adjust(bottom=bottom_margin, right=0.94, top=top_margin)
    except Exception:
        pass
    try:
        os.makedirs(os.path.dirname(out_path) or '.', exist_ok=True)
...
def render_trna_gene_ordered_heatmaps(SET, count_df: pd.DataFrame, ordered_genes, output_dir: str, cluster_df: pd.DataFrame | None = None, selected_clusters=None):
    ...
    # modification enzymes from decoding-table columns 6 and 7.
    if bool(SET.get('trna_modification_heatmap_enable', True)):
        ...
        mod_z_df, mod_label_map = _compute_modification_usage_z(count_df, trna_rules, feature_mode=feature_mode)
        if mod_z_df is not None and not mod_z_df.empty:
            chosen = list(available_clusters)
            mod_rows = list(mod_z_df.columns)
            plot_kind = str(SET.get('trna_modifications_plot_kind', 'heatmap') or 'heatmap').strip().lower()
            mode_label = 'tRNA modification enzymes' if feature_mode in {'enzyme', 'enzymes', 'trme', 'trmes'} else 'tRNA modifications'
            if plot_kind in {'boxplot', 'violin'}:
                out_path = os.path.join(output_dir, f'Plot 6 - {mode_label} shift within clusters {plot_kind}s.png')
                saved = _plot_cluster_feature_boxplots(
                    SET,
                    values_df=mod_z_df,
                    cluster_df=cluster_df,
                    ...
                    out_path=out_path,
                    title_txt='tRNA modifications enrichment vs genome' if 'modification' in mode_label else f'{mode_label} enrichment vs genome',
                    y_label=f'{mode_label} enrichment\nvs genome (z-score)',
                ...
                if saved:
                    out['trna_modification_cluster_shift'] = saved
            else:
                ...
                if not mat.empty:
                    ...
                    mat.index = display
                    out_path = os.path.join(output_dir, f'Plot 6 - {mode_label} shift within clusters heatmap.png')
                    saved = _plot_cluster_shift_heatmap(
                        SET,
                        mat,
                        out_path=out_path,
